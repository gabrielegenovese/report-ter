...
\end{itemize}
    ...
    \resizebox{0.8\textwidth}{!}{%
        \begin{tikzpicture}[node distance={45mm}, thick, main/.style = {draw, circle}] 
            ...
            \draw[->] (n_2) -- node[midway, above, pos=0.5] {receive ack} (n_3);
            \draw[->] (n_3) to [out=150,in=30]  node[midway, above, pos=0.5] {fork\{0,1\} ! release} (n_1);
        \end{tikzpicture}
        }
    % }%
    ...
    \begin{tikzpicture}[node distance={45mm}, thick, main/.style = {draw, circle}] 
        ...
        \draw[->] (n_3) to [out=160,in=20] node[midway, above, pos=0.5] {receive release} (n_1);
    \end{tikzpicture}
    }
    % }%
    \caption{Local view of a fork.}
    \label{graph:forklocal}2
    \end{figure}
\end{frame}
...
\begin{frame}{Characteristics and challenges}
Requirements:
\begin{itemize}
